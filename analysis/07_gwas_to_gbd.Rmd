...
merge_ranges <- function(df) {
  ...
  for (letter in unique(df$letter)) {
    
    # Get and sort ranges for this ICD-10 category
    ranges <- df |>
    ...
    for (k in seq_len(nrow(ranges))[-1]) {
      
      next_range <- ranges[k, ]
      
      # Extend the current range if the ranges overlap
      if (next_range$start_value <= current_range$end_value) {
        
        if (next_range$end_value > current_range$end_value) {
          current_range$end_value <- next_range$end_value
          current_range$end_label <- next_range$end_label
        }
        
      } else {
        
        # No overlap: save current range and start a new one
        output[[length(output) + 1]] <- current_range
        current_range <- next_range
      }
    }
    
    # Save the final range for this letter
    output[[length(output) + 1]] <- current_range
  }
