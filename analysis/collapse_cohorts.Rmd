...
gwas_study_info <- gwas_study_info |>
  ...
  mutate(COHORT = str_replace_all(COHORT, "(Multiple cohorts)", "multiple")) |>
  ...
  mutate(COHORT = str_replace_all(COHORT, "DECODE", "deCODE")) |>
  mutate(COHORT = str_replace_all(COHORT, "deCode", "deCODE")) |>
  mutate(COHORT = str_replace_all(COHORT, "SIMES", "SiMES")) |>
  mutate(COHORT = str_replace_all(COHORT, "HELIX", "Helix")) |>
  mutate(COHORT = str_replace_all(COHORT, "Finngen", "FinnGen")) 
...
# UK Biobank ==  ?UK controls
gwas_study_info |> filter(grepl("UK controls", COHORT)) |> select(1:5)
gwas_study_info |> filter(grepl("UK sCJD cases", COHORT)) |> select(1:5)

gwas_study_info |> filter(grepl("UK controls", COHORT))  |> pull(COHORT)
...
grep("Steno", single_use_cohorts, value = T)
...
gwas_study_info = gwas_study_info |>
  mutate(COHORT = str_replace_all(COHORT, "Nagahama_Study", "Nagahama Study")) |> 
