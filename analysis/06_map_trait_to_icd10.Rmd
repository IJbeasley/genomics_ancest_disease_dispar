...
disease_mapping <- disease_mapping |>
  mutate(
    trait_clean = trait_clean |>
      ...
      str_remove_all( "(?<=Source of report of\\s)[A-Z][0-9]{1,2}(?:\\.[0-9A-Z]+)?\\b") |>
      str_remove_all( "(?<=ICD-10\\s)[A-Z][0-9]{1,2}(?:\\.[0-9A-Z]+)?\\b")  |>
      str_remove_all( "(?<=ICD10 )[^:]+(?=:)|(?<=ICD10 )[^ ]+") |>
      str_remove_all("ICD10")
)
...
disease_mapping <- disease_mapping |>
  mutate(
    trait_clean = trait_clean |>
                  str_remove_all("Behaviour of cancer tumour (-|:)")
)
...
# make lower case - removes need to do case insensitive matching later
disease_mapping <-
  disease_mapping |>
  mutate(trait_clean = tolower(trait_clean),
         trait_context_clean = tolower(trait_context_clean)
)
