---
title: "Get dbGAP ids for studies"
author: "Isobel Beasley"
date: "2025-10-20"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r setup-libraries, message=FALSE}

library(dplyr)
library(data.table)
library(rentrez)
library(purrr)
library(stringr)

```

# dbGAP ids

## Get from full text 

### Functions to extract dbGaP (and other archive) ids from full text

```{r get_full_text_files_fns}

# patterns to extract: 
# get dbgap ids
# get EGA ids
# get JGAS
# get PRJEB / PRJNA ids
patterns <- list(
  dbgap = "\\bphs\\d+(?:\\.v\\d+)?(?:\\.p\\d+)?",
  ega   = "\\bEGA[CDFS]\\d+",
  jgas  = "\\bJGAS\\d+|JGAD\\d+",
  prj   = "\\bPRJEB\\d+|PRJNA\\d+",
  # clinical trial ids
  clinical_trial = "\\bNCT\\d{8}"
)

grep_pattern <- paste(patterns, collapse = "|")

library(tokenizers)

# get sentences with dbgap / ega ids etc. in them
get_grep_sentences <- function(file_path,
                               grep_pattern) {
  
  #browser()
  
  
  txt_in <- suppressWarnings(xml2::read_xml(file_path))
  
  # txt_in <- tryCatch(
  #   xml2::read_xml(file_path),
  #   error = function(e) {
  #     print(paste0("error with ", file_path));
  #     return(NULL)
  #   }
  # )
  
  txt_in <- xml2::read_xml(file_path)
  
  get_patterns <- c(".//*[local-name()='body']//*[local-name()='p']",
                    ".//*[local-name()='abstract']//*[local-name()='p']",
                    ".//*[local-name()='back']//*[local-name()='p']",
                    ".//*[local-name()='fig']//*[local-name()='caption']",
                    ".//*[local-name()='table-wrap']//*[local-name()='p']",
                    ".//*[local-name()='sec' and @sec-type='data-availability']//*[local-name()='p']",
                    ".//*[local-name()='supplementary-material']//*[local-name()='p']",
                    ".//passage/text")
  
  canidate_text <- txt_in |>
                   xml2::xml_find_all(paste(get_patterns, collapse = " | ")) |>
                   # xml2::xml_find_all("//body//p | 
                   #                     //abstract//p |
                   #                    //back//p  |
                   #                    //fig//caption |
                   #                    //table-wrap//p |
                   #                    //sec[@sec-type='data-availability']//p
                   #                    //supplementary-material") |>
                   #xml2::xml_find_all("//body|//back") |>
                   xml2::xml_text()
  
  
  if(length(canidate_text) == 0) {
    print(paste0("No candidate text found in ", file_path))
    return(character(0))
  }
  
  # txt_in <- readLines(file_path,
  #                     warn = FALSE,
  #                     encoding = "UTF-8")
  # tokenize into sentences
  
  
  sentences <- unlist(
  tokenize_sentences(paste(canidate_text, collapse = " "))
)

grep_sentences <- sentences[grepl(grep_pattern, sentences)]

  # sentences <- tokenize_sentences(paste(canidate_text, collapse = "\n"))
  # 
  # grep_sentences <- sentences[grepl(grep_pattern, sentences)]

  # grep_sentences <- tokenize_sentences(paste(candidate_lines, 
  #                                            collapse = "\n"))
  # 
  # sentences <- tokenize_sentences(paste(txt_in, collapse = " "))
  # 
  # sentences <- unlist(tokenize_sentences(txt_in))
  # 
  # dbgap_sentences = sentences[grepl(grep_pattern, sentences)]

  return(grep_sentences)
}


```

### Abstract text file paths to load

```{r get_abstract_text_file_paths}

# get pmids and pmcids of relevant files to load
relevant_ids <- data.table::fread(here::here("output/fulltexts/pmid_to_pmcid_mapping.csv")) 

abstract_files <- list.files(here::here("output/abstracts/"), 
                             pattern = "*.json", 
                             full.names = FALSE
                             ) |>
                   sort()

abstract_pmids = str_remove_all(abstract_files, "_sentences.json$")
abstract_pmids = abstract_pmids[abstract_pmids %in% relevant_ids$PMID]

abstracts <- sapply(abstract_pmids,
                    function(file) {
                      json_data <- jsonlite::fromJSON(here::here(paste0("output/abstracts/", file, "_sentences.json")))
                      
                      abstract_lines <- json_data[json_data != ""]
                      
                    }
                    )

keep_pmid_abstract_sentences <-
purrr::map(abstracts, 
           ~grep(pattern = grep_pattern, 
                x = .x,
                value = T)
           ) |>
purrr::discard(~rlang::is_empty(.x)) 

abstract_sentences_df <- 
  purrr::imap(keep_pmid_abstract_sentences,
              ~data.frame(pmid = .y,
                          sentence = unique(.x),
                        stringsAsFactors = FALSE)
              )|>
  dplyr::bind_rows()
            

convert_pmid_df  <- data.table::fread(here::here("output/fulltexts/pmid_to_pmcid_mapping.csv")) 


abstract_sentences_df =
  left_join(abstract_sentences_df |>
            mutate(pmid = as.integer(pmid)),
            convert_pmid_df |> 
              dplyr::select(pmid = PMID,
                            pmcid =pmcids)
                            ,
            by = "pmid"
            ) 

abstract_sentences_df <- abstract_sentences_df |>
  mutate(
    dbgap_id = str_extract_all(sentence, patterns[["dbgap"]]),
    ega_id = str_extract_all(sentence, patterns[["ega"]]),
    jgas_id = str_extract_all(sentence, patterns[["jgas"]]),
    prj_id = str_extract_all(sentence, patterns[["prj"]]),
    nct_id = str_extract_all(sentence, patterns[["clinical_trial"]])
  )
  

```

### Full text file paths to load 

```{r get_full_text_file_paths}

full_text_files <-
  list.files(here::here("output/fulltexts"), 
             recursive = T, 
             pattern = "\\.xml$") 

# some duplicates come from converted xml files 
# i.e. from propriety wiley and elsevier xml
# let's keep only the converted files:
full_text_files <- 
  grep("elsevier/elsevier_xml|wiley/wiley_xml", 
       full_text_files, 
       value = TRUE,
       invert = TRUE)


# get pmcids of these files
all_ids <-
  full_text_files |>
  gsub(pattern = ".*/", replacement = "") |>
  gsub(pattern = "\\.xml$", replacement = "") |>
  gsub(pattern = "_bioc", replacement = "") |>
  gsub(pattern = "\\.pdf.tei", replacement = "")

# some duplicates come from ncbi and europe pmc
# let's keep the europe pmc files
duplicate_ids <-
data.frame(ids = all_ids) |>
  group_by(ids) |>
  summarise(count = n()) |>
  filter(count > 1) |>
  pull(ids)

grep(full_text_files, 
     pattern = duplicate_ids[1],#paste(duplicate_ids, collapse = "|"),
     value = T)

duplicates_to_remove <-
grep(full_text_files, 
     pattern = paste(duplicate_ids, collapse = "|"),
     value = T) |>
  grep(pattern = "ncbi", value = T)

full_text_files <- full_text_files[!c(full_text_files %in% duplicates_to_remove)]

# convert to file paths 
full_text_files <- here::here(paste0("output/fulltexts/", 
                                     full_text_files)
                              )


relevant_ids <- c(relevant_ids$PMID, relevant_ids$pmcids)

all_text_ids <-   full_text_files |>
  gsub(pattern = ".*/", replacement = "") |>
  gsub(pattern = "\\.xml$", replacement = "") |>
  gsub(pattern = "_bioc", replacement = "") |>
  gsub(pattern = "\\.pdf.tei", replacement = "")

text_files_to_get <-
data.frame(full_text_files = full_text_files,
           all_text_ids = all_text_ids) |>
  filter(all_text_ids %in% relevant_ids)

full_text_files <- text_files_to_get$full_text_files

print("Number of full text files to load:")
print(length(full_text_files))

```

### Extract dbGaP accessions from full text

```{r get_dbgap_from_full_text, warning=FALSE}

pmcid_dbgap_sentences <- 
purrr::map(full_text_files,
           ~get_grep_sentences(.x, 
                               grep_pattern = grep_pattern
                               )
           ) 

all_text_ids <-   full_text_files |>
  gsub(pattern = ".*/", replacement = "") |>
  gsub(pattern = "\\.xml$", replacement = "")  |>
  gsub(pattern = "_bioc", replacement = "") |>
  gsub(pattern = "\\.pdf.tei", replacement = "")

names(pmcid_dbgap_sentences) <- all_text_ids

keep_pmcid_dbgap_sentences <-
  purrr::discard(
pmcid_dbgap_sentences,
~rlang::is_empty(.x)
) 


# Convert to data frame
sentences_df <- 
  purrr::imap(keep_pmcid_dbgap_sentences,
              ~data.frame(pmcid = .y,
                          sentence = unique(.x),
                        stringsAsFactors = FALSE)
              )|>
  dplyr::bind_rows()

# Extract all dbGaP IDs from the sentences
sentences_df <- sentences_df |>
  mutate(
    dbgap_id = str_extract_all(sentence, patterns[["dbgap"]]),
    ega_id = str_extract_all(sentence, patterns[["ega"]]),
    jgas_id = str_extract_all(sentence, patterns[["jgas"]]),
    prj_id = str_extract_all(sentence, patterns[["prj"]]),
    nct_id = str_extract_all(sentence, patterns[["clinical_trial"]])
  )
             
sentences_df = 
  sentences_df |>
  distinct()

convert_pmid_df  <- data.table::fread(here::here("output/fulltexts/pmid_to_pmcid_mapping.csv")) |>
  rename(PMCID = pmcids)

# add pmid column
sentences_df =
  left_join(sentences_df,
            convert_pmid_df |> 
              dplyr::select(pmcid = PMCID,
                            pmid = PMID),
            by = "pmcid"
            ) |>
  mutate(pmid = ifelse(is.na(pmid), 
                      pmcid,
                      pmid
                      )
  ) |> 
  mutate(pmcid = ifelse(grepl("PMC", pmid),
                        pmid,
                        pmcid)
         ) |>
  mutate(pmcid = ifelse(grepl("PMC", pmcid), 
                        pmcid, 
                        ""
                        )
  )



sentences_df = 
  left_join(sentences_df |> 
              select(-pmcid) |> 
              mutate(pmid = as.integer(pmid)),
            convert_pmid_df |> 
              dplyr::select(pmcid = PMCID,
                            pmid = PMID),
            by = "pmid"
            ) 

# bind with abstract sentence:
sentences_df = 
  bind_rows(sentences_df, 
            abstract_sentences_df) |>
  distinct()

print("Number of unique PMCIDs with dbgap / ega ids found in full text:")
sentences_df$pmcid |> unique() |> length()

# number of pubmed ids with dbgap ids found
print("Number of unique PMCIDs with dbgap ids found in full text:")
sentences_df %>%
  rowwise() %>%
  filter(!rlang::is_empty(dbgap_id)) %>%
  filter(length(dbgap_id) > 0) %>%
  ungroup() %>%
  pull(pmcid) %>%
  unique() %>%
  length()

print("Number of unique PMCIDs with EGA ids found in full text:")
sentences_df %>%
  rowwise() %>%
  filter(!rlang::is_empty(ega_id)) %>%
  filter(length(ega_id) > 0) %>%
  pull(pmcid) %>%
  unique() %>%
  length()

# typo in text for PMC8486151
# replace  "phs000" with 
# phs000.56.v1.p1

sentences_df |> filter(pmcid == "PMC8486151") |> pull(dbgap_id)

fix_dbgap_ids <- 
  c(sentences_df |> filter(pmcid == "PMC8486151") |> pull(dbgap_id),
    "phs00056.v1.p1"
    ) |>
  unlist()

fix_dbgap_ids <- fix_dbgap_ids[fix_dbgap_ids != "phs000"]

sentences_df <-
  sentences_df |>
  mutate(dbgap_id = ifelse(pmcid == "PMC8486151",
                          list(fix_dbgap_ids),
                          dbgap_id
                          )
         )
         

data.table::fwrite(sentences_df,
                   here::here("output/gwas_cat/gwas_study_dbgap_ega_sentences.csv")
                   )
                   
                   
```

                  
## Get from Europe PMC list

```{r get-dbgap-ids-europe-pmc}

dbgap_info <-
data.table::fread(here::here("data/europe_pmc/dbgap.csv")) 

pmids <- convert_pmid_df$PMID 

pmcids <- convert_pmid_df$PMCID

relevant_pmid_dbgaps = dbgap_info |>
  filter(EXTID %in% pmids) |>
  distinct() 

```

### Compare dbgap annotations found in Europe PMC vs full text

```{r compare-europe-pmc-dbgap}

print("Number of unique PMIDs with dbgap ids found in Europe PMC:")
relevant_pmid_dbgaps |>
  pull(EXTID) |>
  unique() |>
  length()

relevant_pmcids_dbgaps = dbgap_info |>
  filter(PMCID %in% pmcids) |>
  filter(PMCID != "") |>
  distinct()

print("Number of unique PMCIDs with dbgap ids found in Europe PMC:")
relevant_pmcids_dbgaps |>
  pull(PMCID) |>
  unique() |>
  length()

# overlap between pmid and pmcid dbgap annotations in europe pmc
print("Number of unique PMIDs with dbgap ids found in Europe PMC that also have dbgap ids found in full text:")
unique(sentences_df$pmid) %in% unique(relevant_pmid_dbgaps$EXTID) |>
  sum() 

# overlap between pmid and pmcid dbgap annotations in europe pmc
# 49 / 57 = 86% of pmids with dbgap ids in europe pmc annotations are also found in full text

print("Number of unique PMIDs with dbgap ids found in full text, but not in Europe PMC:")
setdiff(unique(sentences_df$pmid), 
        unique(relevant_pmid_dbgaps$EXTID)) |> length()

# 101 / 158 = 64% of pmids with dbgap ids found in full text are not found in europe pmc annotations

print("Number of unique PMIDs with dbgap ids found in Europe PMC that do not have dbgap ids found in full text:")
setdiff(unique(relevant_pmid_dbgaps$EXTID), 
        unique(sentences_df$pmid)) |> length()

```

```{r compare-europe-pmc-full-text}

full_text_dbgaps = 
sentences_df  |>
  select(dbgap_id, pmid, pmcid) |>
  tidyr::unnest(dbgap_id) |>
  mutate(dbgap_id = str_trim(dbgap_id)) |>
  mutate(dbgap_id = str_extract(dbgap_id, "^phs\\d+")) |>
  # mutate(dbgap_id = str_remove(dbgap_id, "\\.v\\d+\\.p\\d+.*$")) |> 
  group_by(pmid, pmcid) |>
  summarise(dbgap_full_text = str_flatten(unique(sort(dbgap_id)), 
                          collapse = ", ", 
                          na.rm = TRUE)) |>
#  tidyr::nest(dbgap_full_text = dbgap_id) |>
  mutate(pmid = as.character(pmid))


europe_pmc_dbgaps =
relevant_pmid_dbgaps |>
  select(dbgap_id = dbgap,
         pmcid = PMCID,
         pmid = EXTID) |>
    group_by(pmid, pmcid) |>
  summarise(dbgap_epmc = str_flatten(unique(sort(dbgap_id)), 
                          collapse = ", ", 
                          na.rm = TRUE)) 
 # tidyr::nest(dbgap_epmc = dbgap_id) 

joined <-
full_join(full_text_dbgaps,
          europe_pmc_dbgaps,
          by = c("pmid", "pmcid")
          )

# includes more dbgap ids from full text than europe pmc annotations
joined |> 
  filter(!is.na(dbgap_full_text) & !is.na(dbgap_epmc)) |> 
  filter(dbgap_full_text != dbgap_epmc) 

joined |>
  filter(!is.na(dbgap_full_text) & is.na(dbgap_epmc))

# seems good extraction: 
# pubmed id: 39024449
# pubmed id: 26959717
# pubmed id: 31537701
# pubmed id: 37167549
# pubmed id: 30207284
# pubmed id: 33135175
# pubmed id: 37069358

joined |>
  filter(is.na(dbgap_full_text) & !is.na(dbgap_epmc))
```

## Get from Entrez

### Extract internal dbGAP ids

```{r get-dbgap-ids, eval = F}

Sys.getenv("ENTREZ_KEY")

get_internal_dbgap_ids = function(pubmed_id) {
  
  Sys.sleep(0.1)

dbgap_links <- entrez_link(
            dbfrom = "pubmed",
            db = "gap",
            id = pubmed_id
)

dbgap_ids <- unlist(dbgap_links$links$pubmed_gap)

if(is.null(dbgap_ids)){
  dbgap_ids = NA
}

return(data.frame(PUBMED_ID = pubmed_id,
                  DBGAP_ID = str_flatten(dbgap_ids,
                                         collapse = ",",
                                         na.rm = TRUE)
                  )
)

}

pubmed_dbgap_mapping <-
purrr::map(pmids,
           get_internal_dbgap_ids,
           .progress = TRUE
           ) |>
  dplyr::bind_rows()


```

### Save dbGaP id mapping

```{r save-dbgap-ids, eval = F}

data.table::fwrite(pubmed_dbgap_mapping,
                   here::here("output/gwas_cohorts/gwas_study_dbgap_ids.csv")
                   )

```

### Percentage of studies with dbGAP ids

```{r percent-with-dbgap-ids}

# load previously saved mapping
pubmed_dbgap_mapping <- data.table::fread(
  here::here("output/gwas_cohorts/gwas_study_dbgap_ids.csv")
)

n_with_dbgap = pubmed_dbgap_mapping |>
  dplyr::filter(DBGAP_ID != "") |>
  nrow()

print("Number of unique PMIDs with dbgap ids found via Entrez:")
n_with_dbgap

percent_with_dbgap = n_with_dbgap / nrow(pubmed_dbgap_mapping) * 100

print("Percentage of PMIDs with dbgap ids found via Entrez:")
percent_with_dbgap

```

### Extract dbGAP study accessions

```{r extract-dbgap-accessions, warning=FALSE, message=FALSE, eval = F}

entrez_dbgap_ids = pubmed_dbgap_mapping$DBGAP_ID

entrez_dbgap_ids = entrez_dbgap_ids[entrez_dbgap_ids != ""]

entrez_dbgap_ids = entrez_dbgap_ids |>
  strsplit(",") |>
  unlist() |>
  unique()

print("Number of unique internal dbgap ids to retrieve accessions for:")
print(length(entrez_dbgap_ids))


get_dbgap_accession = function(internal_dbgap_id) {
  
  
  summary = entrez_summary(db = "gap", 
                           id = internal_dbgap_id)
  
  
  accession = summary$d_study_results$d_study_id
  
  return(data.frame(
    INTERNAL_DBGAP_ID = internal_dbgap_id,
    DBGAP_ACCESSION = str_flatten(accession, 
                                  collapse = ",", 
                                  na.rm = TRUE)
  )
  )

} 

dbgap_accession_mapping <-
  purrr::map(entrez_dbgap_ids,
             get_dbgap_accession,
             .progress = TRUE
             ) |>
  dplyr::bind_rows()

```


### Save dbGaP accession mapping

```{r save-dbgap-accessions, eval = F}

  data.table::fwrite(
    dbgap_accession_mapping,
     here::here("output/gwas_cohorts/gwas_study_dbgap_accessions.csv")
                     )
  

```

### Percentage of internal dbGaP ids for which we could not retrieve accessions

```{r percent-with-dbgap-accessions}

dbgap_accession_mapping <- data.table::fread(
  here::here("output/gwas_cohorts/gwas_study_dbgap_accessions.csv")
)

# a lot of internal dbgap ids are not able be retrieve study accessions
# sad
# ? perhaps because they are old dbgap versions? 
print("Number of internal dbgap ids for which we could not retrieve accessions:")
dbgap_accession_mapping |> 
  filter(DBGAP_ACCESSION == "") |>
  nrow()


pubmed_dbgap_mapping =
  pubmed_dbgap_mapping |>
  tidyr::separate_longer_delim(cols = "DBGAP_ID", 
                               delim = ",")

dbgap_accession_mapping = 
  dbgap_accession_mapping |>
  tidyr::separate_longer_delim(cols = "INTERNAL_DBGAP_ID", 
                               delim = ",")

dbgap_accession_mapping = 
left_join(pubmed_dbgap_mapping, 
     dbgap_accession_mapping,
     by = c("DBGAP_ID" = "INTERNAL_DBGAP_ID")
     ) 

print("Number of unique PMIDs with dbgap accessions found via Entrez:")
dbgap_accession_mapping |>
  filter(DBGAP_ACCESSION != "" & DBGAP_ID != "") |>
  pull(PUBMED_ID) |>
  unique() |>
  length()

```

### Save dbGaP mapping thus far

```{r save-combined-dbgap-mapping, eval = F}

combined_dbgap_pubmed_mapping = 
  dbgap_accession_mapping |>
  filter(DBGAP_ACCESSION != "" & DBGAP_ID != "") |>
  select(PUBMED_ID, DBGAP_ACCESSION) |>
  distinct()

data.table::fwrite(
  combined_dbgap_pubmed_mapping,
  here::here("output/gwas_cohorts/gwas_study_combined_dbgap_pubmed_mapping.csv")
)

```

### Compare dbgap retrieved via extracting full text vs via entrez

```{r compare-dbgap-sources}

combined_dbgap_pubmed_mapping <- data.table::fread(
  here::here("output/gwas_cohorts/gwas_study_combined_dbgap_pubmed_mapping.csv")
)

entrez_dbgaps <- 
  combined_dbgap_pubmed_mapping |>
  rename(pmid = PUBMED_ID,
         dbgap_id = DBGAP_ACCESSION
         ) |>
  mutate(dbgap_id = str_extract(dbgap_id, "^phs\\d+")) |>
  filter(dbgap_id != "") |>
  group_by(pmid) |>
  summarise(dbgap_annotation = str_flatten(unique(sort(dbgap_id)), 
                          collapse = ", ", 
                          na.rm = TRUE)) |>
  mutate(pmid = as.character(pmid))


joined_dbgaps =
full_join(entrez_dbgaps,
          full_text_dbgaps,
          by = "pmid")

print("Number of PMIDs with dbgap annotations from Entrez that do not match dbgap ids found in full text:")
joined_dbgaps |>
  filter(!is.na(dbgap_annotation) & !is.na(dbgap_full_text)) |>
  filter(dbgap_annotation != dbgap_full_text)
 
print("Number of PMIDs with dbgap annotations from Entrez that match dbgap ids found in full text:")
joined_dbgaps |>
  filter(!is.na(dbgap_annotation) & !is.na(dbgap_full_text)) |>
  filter(dbgap_annotation == dbgap_full_text) |>
  nrow()

print("Number of PMIDs with dbgap annotations from Entrez but no dbgap ids found in full text:")
joined_dbgaps |>
  filter(!is.na(dbgap_annotation) & is.na(dbgap_full_text)) |>
  nrow()

print("Number of PMIDs with dbgap found in full text but not in Entrez annotations:")
joined_dbgaps |>
  filter(is.na(dbgap_annotation) & !is.na(dbgap_full_text)) |>
  nrow()

```

## Combine and save

```{r combine-and-save}

# europe_pmc_dbgaps
# entrez_dbgaps 
# full_text_dbgaps

full_text_dbgaps = 
sentences_df  |>
  select(dbgap_id, pmid, pmcid) |>
  tidyr::unnest(dbgap_id) |>
  mutate(dbgap_id = str_trim(dbgap_id)) |>
  group_by(pmid, pmcid) |>
  summarise(dbgap_full_text = str_flatten(unique(sort(dbgap_id)), 
                          collapse = ", ", 
                          na.rm = TRUE)) |>
  mutate(pmid = as.character(pmid))


europe_pmc_dbgaps =
relevant_pmid_dbgaps |>
  select(dbgap_id = dbgap,
         pmcid = PMCID,
         pmid = EXTID) |>
    group_by(pmid, pmcid) |>
  summarise(dbgap_epmc = str_flatten(unique(sort(dbgap_id)), 
                          collapse = ", ", 
                          na.rm = TRUE)) 

entrez_dbgaps <- 
  combined_dbgap_pubmed_mapping |>
  rename(pmid = PUBMED_ID,
         dbgap_id = DBGAP_ACCESSION
         ) |>
  filter(dbgap_id != "") |>
  group_by(pmid) |>
  summarise(dbgap_annotation = str_flatten(unique(sort(dbgap_id)), 
                          collapse = ", ", 
                          na.rm = TRUE)) |>
  mutate(pmid = as.character(pmid))

combined_annotations <-
full_join(full_text_dbgaps,
          europe_pmc_dbgaps,
          by = c("pmid", "pmcid")
          ) |>
  full_join(entrez_dbgaps,
            by = c("pmid")
            ) |>
  tidyr::separate_longer_delim(cols = c(dbgap_full_text),
                               delim = ", ")|>
  tidyr::separate_longer_delim(cols = c(dbgap_epmc),
                               delim = ", ")|>
  tidyr::separate_longer_delim(cols = c(dbgap_annotation),
                               delim = ", ")|>
  tidyr::pivot_longer(cols = c(dbgap_full_text, dbgap_epmc, dbgap_annotation),
               names_to = "source",
               values_to = "dbgap_id") |>
  filter(!is.na(dbgap_id) & dbgap_id != "") |>
  mutate(source = stringr::str_remove_all("dbgap_", string = source)) |>
  mutate(source = ifelse(source == "annotation", "entrez", source)) |>
  distinct()


combined_annotations$pmid |> unique() |> length()


n_pubmeds_per_dbgap =
combined_annotations |>
  mutate(broad_dbgap = str_extract(dbgap_id, "^phs\\d+")) |>
  select(pmid, broad_dbgap) |>
  distinct() |>
  group_by(broad_dbgap) |>
  summarise(n_pubmeds = n()) |>
  arrange(desc(n_pubmeds))

n_pubmeds_per_dbgap |>
  head(
  )

n_pubmeds_per_dbgap$n_pubmeds |> summary()


```

## Get dbgap sample sizes 

```{r get-dbgap-sample-sizes, eval = F}

library(httr)

all_dbgap_ids <-
combined_annotations |>
  pull(dbgap_id) |> 
  unique() |>
  sort()

# need to have versions and parts to get sample size info from the API
extracted_dbgap_ids <- grep("v\\d+\\.p\\d+", 
                            all_dbgap_ids, 
                            value = T)

# get the relevant information from the API
# API Information / Documentation: https://www.ncbi.nlm.nih.gov/gap/sstr/swagger/
# API extracts this information, for e.g. https://www.ncbi.nlm.nih.gov/gap/sstr/report/phs000387.v1.p1
get_sample_size_df_api <- function(api_link = "https://www.ncbi.nlm.nih.gov/gap/sstr/api/v1/study",
                                   dbgap_id){
  
  url <- paste0(api_link, "/", 
                dbgap_id, 
                "/summary")
  
  response <- httr::GET(url, httr::timeout(60))
  Sys.sleep(0.5)  # be polite to the API
  
  if (httr::status_code(response) != 200) {
    warning("Failed to retrieve data for dbGaP ID ", dbgap_id, 
            ". Status code: ", response$status_code)
    return(NULL)
  }
  
  result_summary <- httr::content(response)
  
  consent_groups_df <- result_summary$study$consent_groups |> 
    dplyr::bind_rows() 
  
  return(consent_groups_df)
  
}

# takes about ~20-30 minutes
dbgap_consent_dfs <- purrr::map(extracted_dbgap_ids,
                                    ~get_sample_size_df_api(dbgap_id = .x),
                                    .progress = TRUE
                                    )

dbgap_consent_dfs <- 
  dbgap_consent_dfs |>
purrr::set_names(extracted_dbgap_ids)

dbgap_consent_dfs <-
  purrr::discard(dbgap_consent_dfs,
                 ~is.null(.x))

# filter out consent code 0 (not used in study)
# and sample_count (counts some individuals twice)
dbgap_consent_n <- purrr::imap(dbgap_consent_dfs,
                                function(x, idx) {
                                  x |>
                                    dplyr::filter(code != 0) |>
                                    dplyr::select(-sample_count) |>
                                    dplyr::mutate(dbgap_id = idx)
                                }
) |>
  dplyr::bind_rows()


dbgap_sample_size <- 
  dbgap_consent_n |>
  dplyr::group_by(dbgap_id) |>
  dplyr::summarise(n_total = sum(subject_count, na.rm = TRUE)) |>
  arrange(dbgap_id)


data.table::fwrite(dbgap_sample_size,
                   here::here("output/gwas_cohorts/dbgap_sample_size.csv"))


```


```{r}


dbgap_sample_size <- data.table::fread(
                   here::here("output/gwas_cohorts/dbgap_sample_size.csv"))


combined_annotations <- 
left_join(
  combined_annotations,
  dbgap_sample_size,
  by = "dbgap_id")


missing_dbgap_sample_size <- 
combined_annotations |>
  filter(is.na(n_total)) |>
  select(pmid, dbgap_id)

print("Number of PUBMED ids for which we could not retrieve sample size information")
missing_dbgap_sample_size$pmid |> unique() |> length()

print("Number of unique dbgap ids for which we could not retrieve sample size information")
missing_dbgap_sample_size$dbgap_id |> unique() |> length()

print("Which PMIDs and dbgaps are missing sample size information?")
combined_annotations |>
  filter(is.na(n_total)) |>
  filter(source !=  "epmc") |> 
  select(pmid, dbgap_id) |>
  distinct()

```


```{r old_dbGaP2x_get_sample_size, eval = F, results='hide', echo = F}

filtered_dbgap_ids <-
  str_extract(all_dbgap_ids, "^phs\\d+") |>
  unique() |>
  sort()

# can only get the latest dbgap accession ids 
latest_dbgap <- purrr::map(filtered_dbgap_ids,
                           ~dbGaP2x::phs.version(.x)
                           ) |> 
  purrr::list_c()

get_total_n_dbgap <- function(dbgap_id) {
  
  n_pop <- tryCatch({
    dbGaP2x::n.pop(dbgap_id)
  }, error = function(e) {
    warning("Error getting n.pop for dbGaP ID ", dbgap_id, ": ", e$message)
      NULL
  })
  
   # Failure path: tryCatch returned NULL
  if (is.null(n_pop) || !is.data.frame(n_pop) || nrow(n_pop) == 0) {
    return(NA)
  }
  
  if(nrow(n_pop) == 1 & ncol(n_pop) == 1 & is.character(n_pop[1,1])){
    warning("n.pop returned a single, non_numeric value for dbGaP ID ", dbgap_id, 
            " which is unexpected. Returning NA.")
    return(NA)
  }
  
  n_total <- n_pop |> 
               dplyr::filter(consent_group == "TOTAL") |>
               pull(total)
  
  if (length(n_total) == 0) {
    warning("No 'TOTAL' consent group found for dbGaP ID ", 
            dbgap_id, 
            ". Returning NA.")
    return(NA)
  }
  
  if(length(n_total) > 1) {
    warning(paste0("Multiple 'Total' consent groups found for dbGaP ID ", dbgap_id, 
                ". Returning NA."))
    return(NA)
  }
  
  return(n_total)
  
}

dbgap_sample_size <- 
  purrr::map(latest_dbgap,
           ~get_total_n_dbgap(.x)) |>
  #purrr::set_names(filtered_dbgap_ids[6:8]) |>
  #purrr::discard(~is.na(.x)) 
  purrr::list_c() |>
  purrr::set_names(latest_dbgap) |>
  tibble::enframe(name = "dbgap_id", 
                  value = "n_total") 


data.table::fwrite(dbgap_sample_size,
                   here::here("output/gwas_cohorts/dbgap_sample_sizes.csv")
                   )


# GET(paste0("http://dbgap-api.ncbi.nlm.nih.gov/fhir/x1/ResearchStudy/,
#                    "phs000019"))

# testing <- xml2::read_xml("https://ftp.ncbi.nlm.nih.gov/dbgap/studies/phs000007/phs000007.v22.p8/GapExchange_phs000007.v22.p8.xml")
# 
# cg <- xml2::xml_find_all(testing, "//ConsentGroup")
# 
# data.frame(
#     consent_group_id   = xml2::xml_attr(cg, "groupNum"),
#     consent_group_name = xml2::xml_attr(cg, "shortName"),
#     stringsAsFactors = FALSE
#   )
# 
#  xml2::xml_find_all(testing, "//NumSubjects")



```

### Are there any dbgap ids not in our cohort description mapping?

```{r new_dbgap_ids, message=FALSE, warning=FALSE}

cohort_mapping <- readxl::read_xlsx(here::here("data/cohort/cohort_desc.xlsx"),
                                 sheet = 1)
annotated_dbgap_ids =
  combined_annotations |>
  pull(dbgap_id) |>
  str_trim() |>
  str_extract(pattern = "^phs\\d+") |>
  unique()

cohort_desc_dbgap_ids = 
  cohort_mapping |>
  pull(dbGaP) |>
  strsplit(",") |>
  unlist() |>
  str_trim() |>
  str_extract(pattern = "^phs\\d+") |> 
  unique()

cohort_desc_dbgap_ids_p2 = 
    cohort_mapping |>
  pull(old_dbGaP) |>
  strsplit(",") |>
  unlist() |>
  str_trim() |>
  str_extract(pattern = "^phs\\d+") |> 
  unique()

cohort_desc_dbgap_ids <- unique(c(cohort_desc_dbgap_ids, 
                                  cohort_desc_dbgap_ids_p2)
                                )

print("Checking that this matching is correct:")
print("I.e. no dgbap ids in cohort description mapping that are not in our annotations:")
not_found_dbgap_ids <- annotated_dbgap_ids[!c(annotated_dbgap_ids %in% cohort_desc_dbgap_ids)]

not_found_dbgap_ids  <- sort(not_found_dbgap_ids)

not_found_dbgap_ids |> length()

not_found_dbgap_ids

```

### Save dbgap accession and matched cohort names

```{r get_cohort_names}

get_dbgap_cohort_names <- 
cohort_mapping |>
  tidyr::separate_longer_delim(cols = c(dbGaP),
                               delim = ",")  |>
  tidyr::separate_longer_delim(cols = c(old_dbGaP),
                               delim = ",")  |>
  tidyr::pivot_longer(cols = c(dbGaP, old_dbGaP),
               names_to = "source",
               values_to = "dbgap_id") |>
  select(cohort, dbgap_id) |>
  mutate(dbgap_id = str_trim(dbgap_id)) |>
  mutate(broad_dbgap = str_extract(dbgap_id, "^phs\\d+")) |>
  filter(!is.na(dbgap_id)) |>
  mutate(cohort = ifelse(is.na(cohort),
                         str_extract(string = dbgap_id,
                                     pattern = "\\bphs\\d+"),
                         cohort)) |>
  select(-dbgap_id) |>
  distinct()

print("Number of unique dbgap ids in cohort description mapping:")
get_dbgap_cohort_names |>
  group_by(broad_dbgap) |>
  summarise(n = n(),
            cohorts = stringr::str_flatten(cohort, collapse = ", ", na.rm = T)) |>
  filter(n > 1) |>
  arrange(desc(n)) |>
  head()

get_dbgap_cohort_names <- 
  get_dbgap_cohort_names |>
  group_by(broad_dbgap) |>
  summarise(cohort = stringr::str_flatten(cohort, collapse = "|", na.rm = T)) 

dbgap_cohort_names =
left_join(
combined_annotations |> 
  mutate(broad_dbgap = str_extract(dbgap_id, "^phs\\d+")),
get_dbgap_cohort_names,
by = c("broad_dbgap")
) 


```



```{r save-dbgap-cohort-names}

data.table::fwrite(dbgap_cohort_names,
                   here::here("output/gwas_cohorts/gwas_study_dbgap_cohort_names.csv")
                   )

```


# EGA ids

## Get EGA ids from Europe PMC annotations

```{r get-ega-ids-europe-pmc}

ega_info <-
data.table::fread(here::here("data/europe_pmc/ega.csv")) 

epmc_ega_ids =
ega_info |>
  filter(EXTID %in% pmids) |>
  select(ega_id = ega, 
         pmid = EXTID,
         pmcid = PMCID) |>
  group_by(pmid, pmcid) |>
  summarise(ega_epmc = str_flatten(unique(sort(ega_id)), 
                          collapse = ", ", 
                          na.rm = TRUE))

```

## Get EGAs from full text 

```{r get-ega-ids-full-text, warning=FALSE, message=FALSE}

full_text_ega_ids =
sentences_df  |>
  select(ega_id, pmid, pmcid) |>
  tidyr::unnest(ega_id) |>
  mutate(ega_id = str_trim(ega_id)) |>
  group_by(pmid, pmcid) |>
  summarise(ega_full_text = str_flatten(unique(sort(ega_id)), 
                          collapse = ", ", 
                          na.rm = TRUE)) |>
  mutate(pmid = as.character(pmid))

```

## Compare EGA ids from full text vs Europe PMC annotations

```{r compare-ega-sources, message=FALSE, warning=FALSE}

joined_ega =
full_join(full_text_ega_ids,
          epmc_ega_ids,
          by = c("pmid", "pmcid")
          )

joined_ega |>
  filter(!is.na(ega_full_text) & !is.na(ega_epmc)) |>
  filter(ega_full_text != ega_epmc)

# european pmcs are a direct subset of full text extracted ega ids, which is good

```

## Are there any EGAs ids not in our cohort description mapping?

```{r get_ega-cohort-names}

cohort_desc_ega_ids = 
  cohort_mapping |>
  pull(EGA) |>
  strsplit(",") |>
  unlist() |>
  str_trim() |>
  str_extract(pattern = "\\bEGA[CDFS]\\d+") |> 
  unique()

cohort_desc_ega_ids  <- cohort_desc_ega_ids[!is.na(cohort_desc_ega_ids)]

full_text_ega_ids <-
joined_ega |>
  pull(ega_full_text) |>
  strsplit(",") |>
  unlist() |>
  str_trim() |>
  str_extract(pattern = "\\bEGA[CDFS]\\d+")

epmc_ega_ids <-
joined_ega |>
  pull(ega_epmc) |>
  strsplit(",") |>
  unlist() |>
  str_trim() |>
  str_extract(pattern = "\\bEGA[CDFS]\\d+")

ega_annotated <- unique(c(full_text_ega_ids, epmc_ega_ids))

ega_annotated <- ega_annotated[!is.na(ega_annotated)]

print("Number of unique EGA ids found in full text and Europe PMC annotations:")
not_found_ega_ids <- ega_annotated[!c(ega_annotated %in% cohort_desc_ega_ids)] 

not_found_ega_ids |> length()

not_found_ega_ids

```

## Add cohort names for EGA ids

```{r get_ega_cohort_names}

ega_ids_cohorts <-
  cohort_mapping |>
  select(cohort, full_name, EGA) |>
  filter(!is.na(EGA)) |>
  mutate(cohort = ifelse(is.na(cohort),
                         str_extract(string = EGA,
                                     pattern = "\\bEGAS\\d+"),
                         cohort)) |>
  tidyr::separate_longer_delim(cols = c(EGA),
                               delim = ",")  |>
  mutate(EGA = str_trim(EGA)) 


joined_ega <-
joined_ega |>
  tidyr::separate_longer_delim(cols = c(ega_full_text, ega_epmc),
                               delim = ", ")  |>
  tidyr::pivot_longer(cols = c(ega_full_text, ega_epmc),
               names_to = "source",
               values_to = "ega_id") |>
  mutate(ega_id = str_trim(ega_id)) |>
  filter(!is.na(ega_id)) |> 
  distinct()

ega_cohort_names =
  left_join(joined_ega,
            ega_ids_cohorts,
            by = c("ega_id" = "EGA"),
            relationship = "many-to-many"
            ) |>
  group_by(pmid, pmcid, source) |>
  summarise(cohort = stringr::str_flatten(unique(cohort), 
                                          collapse = "|", 
                                          na.rm = T
                                          ),
            full_name = stringr::str_flatten(unique(full_name), 
                                            collapse = "|", 
                                            na.rm = T
                                            )
            ) |>
  ungroup() |>
  distinct() 

ega_cohort_names |>
  head()

```

## Save EGA cohort names

```{r save-ega-cohort-names}

data.table::fwrite(ega_cohort_names,
                   here::here("output/gwas_cohorts/gwas_study_ega_cohort_names.csv")
                   )

```


# JGA ids

## Get JGA ids from full text

```{r get-jga-ids-full-text}

jga_ids_full_text =
sentences_df  |>
  select(jgas_id, pmid, pmcid) |>
  tidyr::unnest(jgas_id) |>
  mutate(jgas_id = str_trim(jgas_id))


jga_ids_full_text

```

## Match JGA ids to cohort names

```{r get-jga-cohort-names}

jga_ids_cohorts <-
  cohort_mapping |>
  select(cohort, full_name, JGA) |>
  filter(!is.na(JGA)) |>
  mutate(cohort = ifelse(is.na(cohort),
                         str_extract(string = JGA,
                                     pattern = "\\bJGAS\\d+|JGAD\\d+"),
                         cohort)) |>
  tidyr::separate_longer_delim(cols = c(JGA),
                               delim = ",") |>
  mutate(cohort = ifelse(is.na(cohort),
                         str_extract(string = JGA,
                                     pattern = "\\bJGAS\\d+"),
                         cohort)) |>
  mutate(JGA = str_trim(JGA))

jga_ids_cohorts <-
left_join(
  jga_ids_full_text,
  jga_ids_cohorts,
  by = c("jgas_id" = "JGA")
)

```

## Save JGA cohort names

```{r save-jga-cohort-names}

data.table::fwrite(jga_ids_cohorts,
                   here::here("output/gwas_cohorts/gwas_study_jga_cohort_names.csv")
                   )

```

# Clinical trial ids

## Get clinical trial ids from full text

```{r get-clinical-trial-ids-full-text}

clinical_trial_ids_full_text =
sentences_df  |>
  select(nct_id, pmid, pmcid) |>
  tidyr::unnest(nct_id) |>
  mutate(nct_id = str_trim(nct_id))

print("Number of unique clinical trial ids found in full text:")
nrow(clinical_trial_ids_full_text)

print("Number of unique PMIDs with clinical trial ids found in full text:")
clinical_trial_ids_full_text |> pull(pmid) |> unique() |> length()

```

## Match clinical trial ids to cohort names

```{r get-clinical-trial-cohort-names}

clinical_trial_ids_cohorts <-
  cohort_mapping |>
  select(cohort, full_name, clinical_trial_id) |>
  filter(!is.na(clinical_trial_id)) |>
  mutate(cohort = ifelse(is.na(cohort),
                         str_extract(string = clinical_trial_id,
                                     pattern = "\\bNCT\\d+"),
                         cohort)) |>
  tidyr::separate_longer_delim(cols = c(clinical_trial_id),
                               delim = ",") |>
  mutate(clinical_trial_id = str_trim(clinical_trial_id)) |>
  mutate(cohort = ifelse(is.na(cohort) | cohort == "",
                         str_extract(string = clinical_trial_id,
                                     pattern = "\\bNCT\\d+"),
                         cohort)) |>
  rename(nct_id = clinical_trial_id) |>
  distinct()

clinical_trial_ids_cohorts <- 
left_join(
  clinical_trial_ids_full_text,
  clinical_trial_ids_cohorts,
  by = c("nct_id"),
  relationship = "many-to-many"
)

print("Clinical trials associated with the most PMIDs")
clinical_trial_ids_cohorts |>
  select(nct_id, pmid) |>
  distinct() |>
  group_by(nct_id) |>
  summarise(n_pmids = n()) |>
  arrange(desc(n_pmids)) |>
  head(10)

```

## Get sample size of clinical trials

```{r get_clinical_trial_sample_sizes}

library(ctrdata)

nct_id <- clinical_trial_ids_cohorts$nct_id |> 
           unique() |> 
           sort()

nct_url <- paste0("https://clinicaltrials.gov/expert-search?term=",
              paste0(nct_id, collapse = ","))

query_length = length(nct_id)

result_length <- ctrLoadQueryIntoDb(
  queryterm = nct_url, 
  #register = "EUCTR",
  #register = "CTGOV", # clinicaltrials.gov
  only.count = TRUE
)$n

if(query_length != result_length) {
  warning(paste0("Number of NCT IDs in query (", query_length, 
                 ") does not match number of results returned (", result_length, 
                 "). This may indicate an issue with the query or the data retrieval process."))
}

clinical_trial_test_db <- nodbi::src_duckdb(dbname = "gwas_clinical_trial.duckdb", 
                                            collection = "gwas_catalog"
                                            )
ctrLoadQueryIntoDb(
  queryterm = nct_url,
  con = clinical_trial_test_db
)

clinical_trial_df <- ctrdata::dbGetFieldsIntoDf(
  con = clinical_trial_test_db,
  fields = c(#"protocolSection.conditionsModule.keywords",
             # "protocolSection.identificationModule.secondaryIdInfos.id",
             "protocolSection.identificationModule.briefTitle"
             # "protocolSection.identificationModule.orgStudyIdInfo",
             # "protocolSection.identificationModule.orgStudyIdInfo.id"
             ),
  calculate = c("f.sampleSize", 
                "f.isUniqueTrial", 
                "f.trialTitle")
)

clinical_trial_df <-
  clinical_trial_df |>
  rename(sample_size = .sampleSize,
         is_unique_trial = .isUniqueTrial,
         official_title = .trialTitle,
         brief_title  = protocolSection.identificationModule.briefTitle,
         nct_id = `_id`
         )

print("Clinical trials with the largest sample sizes:")
clinical_trial_df |>
  filter(!is.na(sample_size)) |>
  arrange(desc(sample_size)) |>
  select(brief_title, sample_size) |>
  head(10)

print("Clinical trials we were not able to get sample sizes for:")
clinical_trial_df |>
  filter(is.na(sample_size)) 

full_clinical_trial_info <-
left_join(
  clinical_trial_ids_cohorts,
  clinical_trial_df,
  by = "nct_id"
)
               
```

## Save clinical trial cohort names and sizes

```{r save-clinical-trial-cohort-names}

data.table::fwrite(full_clinical_trial_info,
                   here::here("output/gwas_cohorts/gwas_study_clinical_trial_cohort_names.csv")
                   )

```


