...
# Converting cancer feature terms to cancer names 

```{r cancer_feature_to_names}

gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
        stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("cancer aggressiveness",
                                                "cancer survival",
...
# Adding more specificity to mapped cancer terms 
...
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(collected_all_disease_terms == "cancer" & 
                grepl("ICD10 Z85.4", `DISEASE/TRAIT`, ignore.case = T, fixed = T),
                stringr::str_replace_all(collected_all_disease_terms,
...
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(collected_all_disease_terms == "cancer" & 
                grepl("ICD10 Z85.1", `DISEASE/TRAIT`, ignore.case = T, fixed = T),
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern("cancer"),
...
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(collected_all_disease_terms == "cancer" & 
                grepl("(PheCode 159)", `DISEASE/TRAIT`, ignore.case = T, fixed = T),
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern("cancer"),
...
# from paper: https://pubmed.ncbi.nlm.nih.gov/23300138/
# they study: oesophageal squamous cell (OSCC), pancreatic and hepatocellular cancers
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(collected_all_disease_terms == "cancer" & 
                PUBMED_ID == "23300138",
                "esophageal cancer, pancreatic cancer, liver cancer",
                collected_all_disease_terms
         )
  )


# from paper: https://pubmed.ncbi.nlm.nih.gov/31597885/
# they are most interested in breast cancer
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "31597885",
                str_replace_all( 
                  pattern = vec_to_grep_pattern("cancer"),
                  replacement = "breast cancer",
                  string = collected_all_disease_terms),
                  collected_all_disease_terms
         )
  )

# in breast or colorectal cancer
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "32757270",
                str_replace_all( 
                  pattern = vec_to_grep_pattern("cancer"),
                  replacement = "breast cancer, colorectal cancer",
                  string = collected_all_disease_terms),
                  collected_all_disease_terms
                )
  )
...
# for 36268685
# from paper: https://pubmed.ncbi.nlm.nih.gov/36268685/ studied cancers were:
#lung cancer, malignant melanoma, head and neck cancer, gastric cancer, kidney cancer, Hodgkin’s lymphoma, carcinoma of unknown primary, esophageal cancer, rectal cancer, malignant mesothelioma, bone cancer and gastroesophageal junction cancer
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "36268685",
                str_replace_all( 
                  pattern = vec_to_grep_pattern("cancer"),
                  replacement = "lung cancer, malignant melanoma of skin, head and neck cancer, gastric cancer, kidney cancer, hodgkins lymphoma, carcinoma of unknown primary, esophageal cancer, colorectal cancer, malignant mesothelioma, bone cancer, gastroesophageal junction cancer",
                  string = collected_all_disease_terms),
                  collected_all_disease_terms
                )
  )
...
# 34390503
# indicate focus of study is:
# multiple myeloma, breast cancer, lung cancer, prostate cancer
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "34390503",
                str_replace_all( 
                  pattern = vec_to_grep_pattern("cancer"),
                  replacement = "multiple myeloma, breast cancer, lung cancer, prostate cancer",
                  string = collected_all_disease_terms),
                  collected_all_disease_terms
                )
  )
...
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "37344884",
                str_replace_all( 
                  pattern = vec_to_grep_pattern("cancer"),
                  replacement = "kidney cancer, breast cancer, lung cancer, urothelial cancer, ovarian cancer, malignant melanoma of skin, cervical cancer, endometrial cancer, gastric cancer, head and neck cancer, non-melanoma skin cancer",
                  string = collected_all_disease_terms),
                collected_all_disease_terms
         )
  )
...
gwas_study_info |>
  filter(collected_all_disease_terms == "cancer") |>
  filter(!grepl("ICD", `DISEASE/TRAIT`, ignore.case = TRUE)) |>
  filter(!grepl("PheCode", `DISEASE/TRAIT`, ignore.case = TRUE)) 

studies_with_mapped_cancers <-
 gwas_study_info |>
filter(STUDY_ACCESSION %in% non_specific_cancer$STUDY_ACCESSION) |>
filter(grepl("cancer", collected_all_disease_terms, ignore.case = TRUE))  |>
filter(collected_all_disease_terms != "cancer")

studies_with_mapped_cancers 

# remove non-specific cancer reference for these studies, as specific cancers are mapped in the same study
 gwas_study_info  <-
  gwas_study_info |>
  filter(!(collected_all_disease_terms == "cancer" & 
         STUDY_ACCESSION %in% studies_with_mapped_cancers$STUDY_ACCESSION))
...
non_specific_cancer |>
  filter(grepl("Prostate|PCA3 expression", `DISEASE/TRAIT`, ignore.case = TRUE))  |>
  select(`DISEASE/TRAIT`) |>
  distinct()

gwas_study_info = gwas_study_info |>
  # replace non-specific 'cancer' with 'prostate cancer' where disease/trait contains 'prostate'
  mutate(collected_all_disease_terms = ifelse(grepl("Prostate|PCA3 expression", 
                                                    `DISEASE/TRAIT`, 
                                                    ignore.case = TRUE) &
                                               collected_all_disease_terms == "cancer",
                                 "prostate cancer",
                                 collected_all_disease_terms
                                 )
         )
...
non_specific_cancer |>
  filter(grepl("Breast", `DISEASE/TRAIT`, ignore.case = TRUE))  |>
  select(`DISEASE/TRAIT`) |>
  distinct()

gwas_study_info = gwas_study_info |>
  # replace non-specific 'cancer' with 'breast cancer' where disease/trait contains 'breast'
  mutate(collected_all_disease_terms = ifelse(grepl("Breast", 
                                                    `DISEASE/TRAIT`, 
                                                    ignore.case = TRUE) &
                                               collected_all_disease_terms == "cancer",
                                 "breast cancer",
...
non_specific_cancer |>
  filter(grepl("Bowel|Colon", `DISEASE/TRAIT`, ignore.case = TRUE))  |>
  select(`DISEASE/TRAIT`) |>
  distinct()


gwas_study_info = gwas_study_info |>
  # replace non-specific 'cancer' with 'colorectal cancer' where disease/trait contains 'bowel or colon'
  mutate(collected_all_disease_terms = ifelse(grepl("Bowel|Colon", 
                                                    `DISEASE/TRAIT`, 
                                                    ignore.case = TRUE) &
                                               collected_all_disease_terms == "cancer",
                                 "colorectal cancer",
...
non_specific_cancer |>
  filter(grepl("Lung", `DISEASE/TRAIT`, ignore.case = TRUE))  |>
  select(`DISEASE/TRAIT`) |>
  distinct()

gwas_study_info = gwas_study_info |>
  # replace non-specific 'cancer' with 'lung cancer' where disease/trait contains 'lung'
  mutate(collected_all_disease_terms = ifelse(grepl("Lung", 
                                                    `DISEASE/TRAIT`, 
                                                    ignore.case = TRUE) &
                                               collected_all_disease_terms == "cancer",
                                 "lung cancer",
...
non_specific_cancer |>
  filter(grepl("Skin", `DISEASE/TRAIT`, ignore.case = TRUE))  |>
  select(`DISEASE/TRAIT`) |>
  distinct()

gwas_study_info = gwas_study_info |>
  # replace non-specific 'cancer' with "non-melanoma skin cancer"  and "malignant melanoma of skin" 
  # where disease/trait contains 'skin'
  mutate(collected_all_disease_terms = ifelse(grepl("Skin", 
                                                    `DISEASE/TRAIT`, 
                                                    ignore.case = TRUE) &
                                               collected_all_disease_terms == "cancer",
                                 "non-melanoma skin cancer, malignant melanoma of skin",
                                 collected_all_disease_terms
                                 )
         ) 
...
# 26546620
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = ifelse(PUBMED_ID == "26546620" &
                                               collected_all_disease_terms == "cancer",
                                 "renal cell carcinoma, soft tissue sarcoma, ovarian cancer",
                                 collected_all_disease_terms
                                 )
         )
...
# breast cancer
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = ifelse(PUBMED_ID == "31597885" &
                                               collected_all_disease_terms == "cancer",
                                 "breast cancer",
                                 collected_all_disease_terms
                                 )
...
# prostate cancer
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = ifelse(PUBMED_ID == "34390503" &
                                               collected_all_disease_terms == "cancer",
                                 "multiple myeloma, breast cancer, lung cancer, prostate cancer",
                                 collected_all_disease_terms
                                 )
...
# ovarian cancer 
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = ifelse(PUBMED_ID == "34528705" &
                                               collected_all_disease_terms == "cancer",
                                 "pancreatic cancer, breast cancer,  colorectal cancer, ovarian cancer",
                                 collected_all_disease_terms
                                 )
         )
...
# breast cancer
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = ifelse(PUBMED_ID == "29299148" &
                                               collected_all_disease_terms == "cancer",
...
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = ifelse(PUBMED_ID == "28044437" &
                                               collected_all_disease_terms == "cancer",
                                 "gastrointestinal cancer",
                                 collected_all_disease_terms
                                 )
...
# breast, prostate, uterine, ovarian, and thyroid cancers.
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = ifelse(PUBMED_ID == "35729236" &
                                               collected_all_disease_terms == "cancer",
                                 "breast cancer, prostate cancer, uterine cancer, ovarian cancer, thyroid cancer",
                                 collected_all_disease_terms
                                 )
         )
...
# lymphoma
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = ifelse(PUBMED_ID == "35752658" &
                                               collected_all_disease_terms == "cancer",
                                 "leukemia, lymphoma",
                                 collected_all_disease_terms
                                 )
...
# 37862240
# prostate, head and neck, breast, or lung
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = ifelse(PUBMED_ID == "37862240" &
                                               collected_all_disease_terms == "cancer",
                                 "prostate cancer, head and neck cancer, breast cancer, lung cancer",
                                 collected_all_disease_terms
                                 )
         )
...
#  lung cancer, malignant melanoma, head and neck cancer, gastric cancer, kidney cancer, Hodgkin’s lymphoma, carcinoma of unknown primary, esophageal cancer, rectal cancer, malignant mesothelioma, bone cancer and gastroesophageal junction cancer
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = ifelse(PUBMED_ID == "36268685" &
                                               collected_all_disease_terms == "cancer",
                                 "lung cancer, malignant melanoma of skin, head and neck cancer, gastric cancer, kidney cancer, hodgkins lymphoma, carcinoma of unknown primary, esophageal cancer, rectal cancer, malignant mesothelioma, bone cancer, gastroesophageal junction cancer",
                                 collected_all_disease_terms
                                 )
         )

gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = ifelse(grepl("Illnesses of mother - cancer (UKB data field 20110)",
                                                    `DISEASE/TRAIT`,
                                                    fixed = T) &
                                               collected_all_disease_terms == "cancer",
...
# for 36929942, see table 1
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = ifelse(PUBMED_ID == "36929942" &
                                               collected_all_disease_terms == "cancer",
                                 "breast cancer, colorectal cancer, lung cancer, endometrial cancer, esophageal cancer, glioma, head and neck cancer, malignant melanoma of skin, ovarian cancer, pancreatic cancer, prostate cancer, kidney cancer",
                                 collected_all_disease_terms
                                 )
         )
...
# for pubmed_id: 26811534
# Table 1: https://pmc-ncbi-nlm-nih-gov.ucsf.idm.oclc.org/articles/PMC5070560/#s17
# replace childhood cancer with
...
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "26811534",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("childhood cancer",
                                                "childhood malignant neoplasm")),
                          "non-hodgkins lymphoma, hodgkins lymphoma, sarcoma, leukemia, other cancer"
         ),
         collected_all_disease_terms
         )
  )

# for pubmed id 29059430
# Table 1: https://pmc-ncbi-nlm-nih-gov.ucsf.idm.oclc.org/articles/PMC6059172/#sec9
# replace childhood cancer with
# "retinoblastoma, leukemia, hodgkins lymphoma, non-hodgkins lymphoma, neuroblastoma, soft tissue sarcoma, central nervous system cancer, bone cancer, kidney cancer, other cancer"
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "29059430",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("childhood cancer",
                                                "childhood malignant neoplasm")),
                          "retinoblastoma, leukemia, hodgkins lymphoma, non-hodgkins lymphoma, neuroblastoma, soft tissue sarcoma, central nervous system cancer, bone cancer, kidney cancer, other cancer"
...
# for 26237429
# https://pmc-ncbi-nlm-nih-gov.ucsf.idm.oclc.org/articles/instance/4552570/bin/NIHMS707555-supplement-2.pdf
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "26237429",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("childhood cancer",
                                                "childhood malignant neoplasm")),
                          "leukemia, hodgkins lymphoma, non-hodgkins lymphoma, osteosarcoma, rhabdomyosarcoma, ewing sarcoma, hepatoblastoma, neuroblastoma, kidney cancer, sarcoma, other cancer"
         ),
         collected_all_disease_terms
         )
  )

# for pubmed id: 25963547
# table 1. https://acsjournals.onlinelibrary.wiley.com/doi/10.1002/cncr.29153
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "25963547",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("childhood cancer",
                                                "childhood malignant neoplasm")),
                          "leukemia, central nervous system cancer, hodgkins lymphoma, non-hodgkins lymphoma, soft tissue sarcoma, neuroblastoma, bone cancer, kidney cancer, other cancer"
         ),
         collected_all_disease_terms
         )
  )

# for pubmed id: 36699085 
# Osteosarcoma, medulloblastoma
# table 1. https://pmc-ncbi-nlm-nih-gov.ucsf.idm.oclc.org/articles/PMC9870026/
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "36699085",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("childhood cancer",
                                                "childhood malignant neoplasm")),
                          "osteosarcoma, medulloblastoma, other cancer"
         ),
         collected_all_disease_terms
         )
  )
...
# pubmed id: 33338273
# table 1. https://pmc-ncbi-nlm-nih-gov.ucsf.idm.oclc.org/articles/PMC8044050/
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "33338273",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("childhood cancer",
                                                "childhood malignant neoplasm")),
                          "leukemia, hodgkins lymphoma, non-hodgkins lymphoma, neuroblastoma, soft tissue sarcoma, central nervous system cancer, kidney cancer, other cancer"
         ),
         collected_all_disease_terms
         )
  )
...
# pubmed id: 38652878
# table 1: https://pmc-ncbi-nlm-nih-gov.ucsf.idm.oclc.org/articles/PMC3088729/#S20

gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "38652878",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("childhood cancer",
                                                "childhood malignant neoplasm")),
                          "leukemia, hodgkins lymphoma, non-hodgkins lymphoma, neuroblastoma, retinoblastoma, central nervous system cancer, osteosarcoma, kidney cancer, germ cell tumor, hepatoblastoma, rhabdomyosarcoma, soft tissue sarcoma, ewing sarcoma, other cancer"
         ),
         collected_all_disease_terms
         )
  )

# 34286861 uses same cohort as 38652878
# https://pmc-ncbi-nlm-nih-gov.ucsf.idm.oclc.org/articles/PMC8516694/
# so replace in same way
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "34286861",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("childhood cancer",
                                                "childhood malignant neoplasm")),
                          "leukemia, hodgkins lymphoma, non-hodgkins lymphoma, neuroblastoma, retinoblastoma, central nervous system cancer, osteosarcoma, kidney cancer, germ cell tumor, hepatoblastoma, rhabdomyosarcoma, soft tissue sarcoma, ewing sarcoma, other cancer"
...
# for pubmed id: 35313970
# https://pmc-ncbi-nlm-nih-gov.ucsf.idm.oclc.org/articles/PMC8939156/#Sec9 
# table 1
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "35313970",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("childhood cancer",
                                                "childhood malignant neoplasm")),
                          "leukemia, hodgkins lymphoma, non-hodgkins lymphoma, neuroblastoma, ewing sarcoma, osteosarcoma, rhabdomyosarcoma, sarcoma, hepatoblastoma, retinoblastoma, central nervous system cancer, bone cancer, kidney cancer, germ cell tumor, malignant melanoma of skin, other cancer"
         ),
         collected_all_disease_terms
         )
  )

# for pubmed id: 36508697
# table 1
# https://www.ncbi.nlm.nih.gov/core/lw/2.0/html/tileshop_pmc/tileshop_pmc_inline.html?title=Click%20on%20image%20to%20zoom&p=PMC3&id=10043563_jco-41-1758-g001.jpg
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "36508697",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("childhood cancer",
                                                "childhood malignant neoplasm")),
                          "leukemia, hodgkins lymphoma, non-hodgkins lymphoma, bone cancer, soft tissue sarcoma, other cancer"
         ),
         collected_all_disease_terms
         )
  )

# for pubmed id: 34262104
# sup table 1.
# https://static-content.springer.com/esm/art%3A10.1038%2Fs41698-021-00178-z/MediaObjects/41698_2021_178_MOESM1_ESM.pdf
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "34262104",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("childhood cancer",
                                                "childhood malignant neoplasm")),
                          "hodgkins lymphoma, central nervous system cancer, non-hodgkins lymphoma, bone cancer, soft tissue sarcoma, other cancer, neuroblastoma, hepatoblastoma, osteosarcoma, germ cell tumor"
         ),
         collected_all_disease_terms
         )
  )

# For pubmed id: 38729340
# from table 1. https://pmc.ncbi.nlm.nih.gov/articles/PMC11425505/#S11
# cancers studied are: 
# Leukaemia, Hodgkin's lymphoma, Non-Hodgkin's lymphoma, Brain tumour, Neuroblastoma
# Renal tumour, Hepatic carcinoma, Thyroid carcinoma, Colon carcinoma, Osteosarcoma, Ewing sarcoma, Soft tissue sarcoma, Germ cell tumour
# Skin cancer (incl. melanoma), Retinoblastoma
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(PUBMED_ID == "38729340",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern(c("childhood cancer",
                                                "childhood malignant neoplasm")),
                          "leukemia, hodgkins lymphoma, non-hodgkins lymphoma, brain tumor, neuroblastoma, kidney cancer, liver cancer, thyroid cancer, colorectal cancer, osteosarcoma, ewing sarcoma, soft tissue sarcoma, germ cell tumor, malignant melanoma of skin, non-melanoma skin cancer, retinoblastoma"
         ),
         collected_all_disease_terms
         )
  )
...
# assume where measure is grade, it is cancer
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
...
# Adult diffuse glioma - assume malignant
...
# majority (~90%) graded glioma, glioblastoma and Oligodendroglioma
# so assume malignant
gwas_study_info =
gwas_study_info |>
  mutate(collected_all_disease_terms  =
...
# pubmed id: 34319593
# considers glioma a malignant tumor
gwas_study_info =
gwas_study_info |>
  mutate(collected_all_disease_terms  =
...
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms  = 
         ifelse(STUDY_ACCESSION == "GCST90041889",
                stringr::str_replace_all(collected_all_disease_terms,
                          vec_to_grep_pattern("malignant laryngeal neoplasm"),
                          "larynx cancer"),
                collected_all_disease_terms
         ))
...
### Nasopharyngeal neoplasm -> "nasopharyngeal cancer"
...
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = 
        str_replace_all(collected_all_disease_terms,
                       pattern = "(?<=^|, )testicular neoplasm(?=,|$)",
                       replacement = "testicular cancer")
  )
...
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms = 
        str_replace_all(collected_all_disease_terms,
                       pattern = vec_to_grep_pattern("malignant urinary system neoplasm"),
                       replacement = "urogenital cancer")
  )
...
bone_cancer_terms <- get_descendants(url)$terms
...
# maybe do urinary bladder cancer instead

bladder_cancer_terms <- get_descendants(url)$terms
...
benign_blood_vessel_terms <- get_descendants(url)$terms
...
before <- gwas_study_info$collected_all_disease_terms
...
gwas_study_info <- gwas_study_info |>
  mutate(collected_all_disease_terms = str_replace_all(
      collected_all_disease_terms,
      "(?<=^|, )benign(?: [^,]+)? neoplasm(?=,|$)|(?<=^|, )benign neoplasm of [^,]+(?=,|$)|(?<=^|, )[^,]+ benign neoplasm(?=,|$)",
      "benign neoplasm"))

after <- gwas_study_info$collected_all_disease_terms

stopifnot(!any(grepl("malignant", before) & after == "benign neoplasm"))

# then handle the malignant ones explicitly, e.g.
malignant_nos <- c("secondary malignant neoplasm", "metastatic malignant neoplasm")
gwas_study_info <- gwas_study_info |>
  mutate(collected_all_disease_terms = str_replace_all(
      collected_all_disease_terms, vec_to_grep_pattern(malignant_nos),
...
url <- "http://www.ebi.ac.uk/ols4/api/ontologies/mondo/terms/http%253A%252F%252Fpurl.obolibrary.org%252Fobo%252FMONDO_0002714/descendants"

cns_cancer_terms <- get_descendants(url)$terms
...
cervical_cancer_terms <- get_descendants(url)$terms
...
colorectal_cancer_terms <- get_descendants(url)$terms
...
endometrial_cancer_terms <- get_descendants(url)$terms
...
ocular_cancer_terms <- get_descendants(url)$terms
...
kidney_cancer_terms <- get_descendants(url)$terms
...
leukemia_terms <- get_descendants(url)$terms
...
ids <- ontologyIndex::get_descendants(efo, 
                                      roots = "efo:EFO_0005570",
                                      exclude_roots = FALSE)

lip_oral_cavity_cancer_terms <- stringr::str_squish(tolower(efo$name[ids]))
...
liver_cancer_terms <- get_descendants(url)$terms
...
lung_cancer_terms <- get_descendants(url)$terms
...
gwas_study_info |>
    filter(grepl("\\bsmall cell cancer", 
                 collected_all_disease_terms)) |>
    select(`DISEASE/TRAIT`, collected_all_disease_terms) |>
    distinct()
...
ovarian_cancer_terms <- get_descendants(url)$terms
...
pancreatic_cancer_terms <- get_descendants(url)$terms
...
peripheral_nervous_system_cancer_terms <- get_descendants(url)$terms
...
prostate_cancer_terms <- get_descendants(url)$terms
...
nhl_terms <- get_descendants(url)$terms
...
gwas_study_info |> 
 mutate(collected_all_disease_terms  = 
          stringr::str_replace_all(collected_all_disease_terms ,
                                  pattern = vec_to_grep_pattern("non-hodgkin lymphoma"),
                                   "non-hodgkins lymphoma"
                          )  
        )
...
non_melanoma_skin_cancer_terms <- get_descendants(url)$terms
...
gwas_study_info <- gwas_study_info |>
  mutate(collected_all_disease_terms = str_replace_all(
      collected_all_disease_terms, vec_to_grep_pattern("ocular melanoma"), "ocular cancer"))
...
other_pharynx_cancer_terms <- get_descendants(url)$terms
...
# EFO_1001968 is now obsolete, so we use relevant archive version of EFO ontology to get descendants of EFO_1001968
ids <- ontologyIndex::get_descendants(efo, 
                                      roots = "efo:EFO_1001968",
                                      exclude_roots = FALSE)

soft_tissue_sarcoma_terms <- stringr::str_squish(tolower(efo$name[ids]))
...
stomach_cancer_terms <- get_descendants(url)$terms
...
thyroid_cancer_terms <- get_descendants(url)$terms
...
# Other - collapsing studies where the same trait is labelled twice 
...
# Non-follicular lymphoma (UKB data field 40006) likely non-hodgkins lymphoma
# as ICD10 C83: Non-follicular lymphoma is non-hodgkins lymphoma
...
# includes both hodgkin and non-hodgkins lymphoma
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms =
         ifelse(grepl("lymphoma", collected_all_disease_terms, ignore.case = T) & 
                !grepl("hodgkin", collected_all_disease_terms, ignore.case = T) & 
...
# thus includes both hodgkin and non-hodgkins lymphoma
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms =
         ifelse(grepl("lymphoma", collected_all_disease_terms, ignore.case = T) & 
                !grepl("hodgkin", collected_all_disease_terms, ignore.case = T) & 
...
# perhaps need to read further in, but seems like it is both hodgkin and non-hodgkins lymphoma
gwas_study_info = gwas_study_info |>
  mutate(collected_all_disease_terms =
         ifelse(grepl("lymphoma", collected_all_disease_terms, ignore.case = T) & 
                !grepl("hodgkin", collected_all_disease_terms, ignore.case = T) & 
...
tidy_terms <- function(x){
  x |> str_replace_all(",\\s*,", ",") |>
       ...
       str_replace_all("\\s*,\\s*$", "") |>
       str_squish() |>
       vapply(\(s) paste(unique(str_trim(str_split_1(s, ",\\s*"))), collapse = ", "), character(1))
}

gwas_study_info$collected_all_disease_terms = tidy_terms(gwas_study_info$collected_all_disease_terms)
...
fwrite(gwas_study_info,
      here::here("output/gwas_cat/gwas_study_cancer_group.csv")
         )
