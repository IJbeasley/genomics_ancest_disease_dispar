...
* Needs to be run UCSF Network or UCSF VPN to access Elsevier API for subscribed content.
...
```{bash convert_elsevier_to_jats, echo = FALSE}
...
# drop PMIDs with that have been downloaded from elsevier, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/elsevier/elsevier_xml/"),
             pattern = "\\.xml$"
             ) |> stringr::str_remove_all("\\.xml$")

doi_information <-
  doi_information |>
  filter(!PMID %in% pmids_to_drop)
...
# drop PMIDs with that have been downloaded from sage, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/sage"),
             pattern = "\\.xml$"
) |> stringr::str_remove_all("\\.xml$")
...
# drop PMIDs with that have been downloaded from springer nature, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/springer_nature"),
                  recursive = TRUE,
             pattern = "\\.html$"
) |> 
  basename() |>
...
# drop PMIDs with that have been downloaded from wiley, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/wiley/"), 
                  recursive = FALSE,
                  pattern = "\\.xml$") |>
  basename() |>
  stringr::str_remove_all("\\.xml$") |>
...
pmids_to_drop <- list.files(here::here("output/fulltexts/ajcr"),
                  pattern = "\\.xml$"
) |>
  basename() |>
  stringr::str_remove_all("\\.xml$") |>
...
# drop PMIDs with that have been downloaded from ASM, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/asm"),
                  recursive = TRUE,
             pattern = "\\.xml$"
) |>
  basename() |>
...
# drop PMIDs with that have been downloaded from BMJ, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/bmj"),
                  recursive = TRUE,
             pattern = "\\.xml$"
) |>
  basename() |>
...
# drop PMIDs with that have been downloaded from Cambridge, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/cambridge"),
                  recursive = TRUE,
             pattern = "\\.xml$"
) |>
  basename() |>
...
# drop PMIDs with that have been downloaded from ERS, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/ers"),
                  recursive = TRUE,
             pattern = "\\.xml$"
) |>
  basename() |>
...
# drop PMIDs with that have been downloaded from eScholarship, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/escholarship"),
                  pattern = "\\.xml$"
) |>
  basename() |>
  stringr::str_remove_all("\\.xml$") |>
...
# drop PMIDs with that have been downloaded from JAMA, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/jama"),
                  recursive = TRUE,
             pattern = "\\.xml$"
) |>
  basename() |>
...
# drop PMIDs with that have been downloaded from In vivo, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/in_vivo"),
                  recursive = TRUE,
             pattern = "\\.xml$"
) |>
  basename() |>
...
# drop PMIDs with that have been downloaded from Karger, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/karger"),
                  recursive = TRUE,
             pattern = "\\.xml$"
) |>
  basename() |>
...
# drop PMIDs with that have been downloaded from Medica, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/medica"),
                  recursive = TRUE,
             pattern = "\\.xml$"
) |>
  basename() |>
...
# drop PMIDs with that have been downloaded from Ovid Lippincott Williams & Wilkins Total Access Collection, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/ovid"),
                  recursive = TRUE,
             pattern = "\\.xml$"
) |>
  basename() |>
...
# drop PMIDs with that have been downloaded from Oxford Academic, so we don't try to download them again from other publishers
pmids_to_drop <- list.files(here::here("output/fulltexts/oxford_academic"),
                  recursive = TRUE,
             pattern = "\\.xml$"
) |>
  basename() |>
...
pmids_to_drop <- list.files(here::here("output/fulltexts/taylor_and_francis"),
                  recursive = TRUE,
             pattern = "\\.xml$"
) |>
  basename() |>
...
# drop archived files:
full_text_files <- full_text_files[!grepl("^archived/", full_text_files)]
